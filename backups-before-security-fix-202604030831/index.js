...
function mergeStateByRole(currentState, incomingState, actor) {
  ...
  if (actor.role === 'superadmin') {
    ...
    // دمج بيانات المدارس (rewardStore, messaging)
    merged.schools = incoming.schools.map((inSchool) => {
      const curSchool = currentState.schools.find((s) => s.id === inSchool.id);
      if (!curSchool) return inSchool;
      return {
        ...inSchool,
        rewardStore: {
  ...
  if (actor.role === 'principal') {
    ...
    if (incomingSchool) {
      ...
      const mergedSchool = {
        ...incomingSchool,
        // الحفاظ على بيانات الخادم الأحدث للرسائل والمتجر
        messaging: currentSchool?.messaging || incomingSchool.messaging,
        rewardStore: {
