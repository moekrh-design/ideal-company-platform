...
function LeavePassesPage({ selectedSchool, currentUser, users, initialPassId, onCreateLeavePass, onSendLeavePass, onMarkViewed, onUpdateLeavePassStatus, onAssignTeacher, viewMode = "main" }) {
  ...
  // المعلم يرى فقط الطلبات التي لم يتصرف بها بعد (لم تصل لـ released-teacher أو أعلى)
  const teacherInbox = useMemo(() => leavePasses.filter((item) => {
    const myId = String(currentUser?.id || '');
    const myName = String(currentUser?.name || currentUser?.fullName || currentUser?.username || '');
    // مطابقة بالـ ID أو بالاسم (للاستئذانات القادمة من ولي الأمر بدون teacherUserId)
    const isMyPass = (String(item.teacherUserId || '') !== '' && String(item.teacherUserId || '') === myId)
      || (String(item.teacherUserId || '') === '' && myName && String(item.teacherName || '') === myName);
    if (!isMyPass) return false;
    // أخفِ الطلبات التي تصرف بها المعلم (released-teacher/completed/cancelled)
    if (['released-teacher', 'completed', 'cancelled'].includes(String(item.status || ''))) return false;
    return true;
  }), [leavePasses, currentUser]);
  ...
  const [sentPassIds, setSentPassIds] = React.useState(new Set());
  // === اختيار المعلم للاستئذانات القادمة من ولي الأمر ===
  const [assignTeacherPassId, setAssignTeacherPassId] = React.useState(null);
  const [assignTeacherUserId, setAssignTeacherUserId] = React.useState('');
  const [assigningTeacher, setAssigningTeacher] = React.useState(false);
  const [assignTeacherStatus, setAssignTeacherStatus] = React.useState('');
  ...
  const boardRows = useMemo(() => {
    const rows = dashboardRows.filter((item) => {
      ...
      if (boardFilter === 'needsClosure') return ['viewed', 'approved-agent', 'approved-counselor', 'released-guardian', 'released-teacher'].includes(String(item.status || ''));
      // للمعلم: لا يُظهر الطلبات التي تصرف بها (released-teacher وما فوق)
      if (String(currentUser?.role || '') === 'teacher') return !['released-teacher', 'completed', 'cancelled'].includes(String(item.status || ''));
      return !['completed', 'cancelled'].includes(String(item.status || ''));
    });
    // ترتيب: released-teacher أولاً (بانتظار الإقفال) ثم الباقي حسب الوقت
    rows.sort((a, b) => {
      const aUrgent = String(a.status || '') === 'released-teacher' ? 0 : 1;
      const bUrgent = String(b.status || '') === 'released-teacher' ? 0 : 1;
  ...
  const boardSummary = useMemo(() => ({
    ...
    needsClosure: dashboardRows.filter((item) => ['viewed', 'approved-agent', 'approved-counselor', 'released-guardian', 'released-teacher'].includes(String(item.status || ''))).length,
  ...
  const handleCreate = async () => {
    ...
    if (result?.ok && result?.leavePass?.id) {
      ...
      // إرسال تلقائي بناءً على طريقة الإرسال المختارة
      if (form.sendChannel === 'system') {
        // تمرير leavePass object مباشرة لتجنب مشكلة state غير محدث
        const sendResult = await onSendLeavePass?.(result.leavePass, 'system');
        if (!sendResult?.ok) {
          window.alert(sendResult?.message || 'تم إنشاء الاستئذان لكن تعذر إرساله للمعلم.');
        }
        // لا نُظهر رسالة نجاح - نكتفي بتحديث الواجهة
      } else if (form.sendChannel === 'manual') {
        // تمرير leavePass object مباشرة
        const sendResult = await onSendLeavePass?.(result.leavePass, 'manual');
        if (sendResult?.ok && sendResult?.whatsAppUrl) {
          window.open(sendResult.whatsAppUrl, '_blank');
        }
        // لا نُظهر alert - الواتساب يفتح تلقائياً
      }
      // إخفاء نموذج الإنشاء بعد الإنشاء الناجح
      setForm({ studentId: '', teacherUserId: '', destination: 'agent', reason: '', note: '', guardianName: '', guardianMobile: '', sendChannel: 'system' });
    } else {
  ...
  const sendSystem = async (pass) => {
    ...
    setSendPassStatus('');
    const result = await onSendLeavePass?.(pass, 'system');
  ...
  // === دالة تعيين المعلم للاستئذان ===
  const assignTeacherToPass = async (pass) => {
    if (!assignTeacherUserId) return window.alert('يرجى اختيار المعلم أولاً.');
    const teacher = teacherUsers.find((u) => String(u.id) === String(assignTeacherUserId));
    if (!teacher) return window.alert('المعلم غير موجود.');
    setAssigningTeacher(true);
    setAssignTeacherStatus('');
    const teacherName = teacher.name || teacher.fullName || teacher.username;
    try {
      // 1. تحديث بيانات الاستئذان عبر API
      const res = await fetch(`/api/leave-passes/${pass.id}/assign-teacher`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ teacherUserId: teacher.id, teacherName }),
      });
      const data = await res.json();
      if (data.ok) {
        // 2. تحديث الستيت المحلي فوراً عبر prop
        await onAssignTeacher?.(pass.id, teacher.id, teacherName);
        setAssignTeacherStatus('✅ تم تعيين المعلم بنجاح');
        setAssignTeacherPassId(null);
        setAssignTeacherUserId('');
        // 3. إرسال إشعار للمعلم تلقائياً بعد تحديث الستيت
        setTimeout(async () => {
          await sendSystem({ ...pass, teacherUserId: String(teacher.id), teacherName });
        }, 600);
      } else {
        setAssignTeacherStatus('❌ ' + (data.message || 'تعذر تعيين المعلم'));
      }
    } catch (e) {
      setAssignTeacherStatus('❌ خطأ في الاتصال بالخادم');
    }
    setAssigningTeacher(false);
    setTimeout(() => setAssignTeacherStatus(''), 5000);
  };
  ...
  const principalColumns = [
    ...
    ...(isTeacher ? [{ key: 'actions', label: 'الإجراء', render: (row) => (
      <div className="flex flex-wrap gap-1.5">
        ...
        <button onClick={async (e) => { e.stopPropagation(); setCompletionEffect({ id: row.id, type: 'exit', studentName: row.studentName }); await onUpdateLeavePassStatus?.(row.id, 'completed'); setTimeout(() => { setRecentlyCompleted((prev) => new Set([...prev, row.id])); setCompletionEffect(null); }, 2500); }} className="inline-flex items-center gap-1 rounded-xl bg-emerald-600 px-3 py-1.5 text-xs font-bold text-white"><ClipboardCheck className="h-3 w-3" /> خرج من الفصل</button>
      </div>
    ) }] : []),
    ...(!isTeacher && (canCreate || viewMode === 'agent' || viewMode === 'counselor') ? [{ key: 'adminActions', label: 'الإجراء', render: (row) => (
      <div className="flex flex-wrap gap-1.5">
        ...
        {String(row.status || '') === 'released-teacher' ? (
          <button onClick={async (e) => { e.stopPropagation(); setCompletionEffect({ id: row.id, type: 'complete', studentName: row.studentName }); await onUpdateLeavePassStatus?.(row.id, 'completed'); setTimeout(() => { setRecentlyCompleted((prev) => new Set([...prev, row.id])); setCompletionEffect(null); }, 2500); }} className="inline-flex items-center gap-1 rounded-xl bg-teal-600 px-2 py-1 text-xs font-bold text-white"><ClipboardCheck className="h-3 w-3" /> تأكيد الخروج</button>
        ) : (
  ...
  return (
    <div className="space-y-6">
      ...
      {/* === قسم الطلبات بانتظار الإقفال - يظهر للمدير والأمن فقط === */}
      {String(currentUser?.role || '') !== 'teacher' && (() => {
        const urgentPasses = leavePasses.filter((item) => String(item.status || '') === 'released-teacher');
        if (!urgentPasses.length) return null;
        return (
          <div className="rounded-3xl border-2 border-rose-400 bg-rose-50 p-5">
            <div className="mb-4 flex items-center gap-3">
              <div className="flex h-10 w-10 items-center justify-center rounded-2xl bg-rose-600 text-white text-lg">🚨</div>
              <div>
                <div className="text-base font-black text-rose-900">طلبات بانتظار الإقفال</div>
                <div className="text-xs text-rose-600">خرج الطالب من الفصل — يحتاج تأكيد الخروج من الإدارة أو الأمن</div>
              </div>
              <span className="mr-auto inline-flex min-w-8 items-center justify-center rounded-full bg-rose-600 px-3 py-1 text-sm font-black text-white">{urgentPasses.length}</span>
            </div>
            <div className="grid gap-3 md:grid-cols-2 lg:grid-cols-3">
              {urgentPasses.map((pass) => (
                <div key={pass.id} className="rounded-2xl border-2 border-rose-300 bg-white p-4">
                  <div className="mb-2 flex items-start justify-between gap-2">
                    <div>
                      <div className="font-black text-slate-900">{pass.studentName}</div>
                      <div className="text-xs text-slate-500">{pass.className || pass.companyName || '—'} — {pass.teacherName || '—'}</div>
                    </div>
                    <span className="rounded-full bg-rose-100 px-2 py-0.5 text-[11px] font-black text-rose-700 ring-1 ring-rose-200">خرج من الفصل</span>
                  </div>
                  <div className="mb-3 text-xs text-slate-600">{getLeavePassDestinationLabel(pass.destination)} · {getLeavePassElapsedLabel(pass)}</div>
                  <button
                    onClick={async () => { await onUpdateLeavePassStatus?.(pass.id, 'completed'); }}
                    className="w-full rounded-2xl bg-rose-600 py-2 text-sm font-black text-white hover:bg-rose-700"
                  >
                    ✓ تأكيد الخروج وإقفال الطلب
                  </button>
                </div>
              ))}
            </div>
            ...
            <div className="rounded-3xl bg-slate-50 p-4 ring-1 ring-slate-200">
              ...
              <div className="grid gap-3 md:grid-cols-2">
                {boardRows.length ? boardRows.slice(0, 8).map((pass) => (
                  <div key={pass.id} className={cx('rounded-3xl border p-4 text-right transition hover:shadow-sm', String(pass.status || '') === 'released-teacher' ? 'border-rose-400 bg-rose-50 ring-2 ring-rose-400' : (pass.queueMeta?.cardClass || 'border-slate-200 bg-white'), String(selectedId) === String(pass.id) ? 'ring-2 ring-sky-300' : '')}>
                    <button onClick={() => setSelectedId(String(pass.id))} className="w-full text-right">
                      <div className="flex items-start justify-between gap-3">
                        ...
                        <Badge tone={String(pass.status || '') === 'released-teacher' ? 'rose' : (pass.queueMeta?.tone || 'blue')}>{String(pass.status || '') === 'released-teacher' ? '🚨 بانتظار الإقفال' : (pass.queueMeta?.label || 'جديد')}</Badge>
                      </div>
                    ...
                    {String(pass.status || '') === 'released-teacher' && String(currentUser?.role || '') !== 'teacher' && (
                      <div className="mt-3 pt-3 border-t border-rose-200">
                        <button
                          onClick={async (e) => { e.stopPropagation(); setCompletionEffect({ id: pass.id, type: 'complete', studentName: pass.studentName }); await onUpdateLeavePassStatus?.(pass.id, 'completed'); setTimeout(() => { setRecentlyCompleted((prev) => new Set([...prev, pass.id])); setCompletionEffect(null); }, 2500); }}
                          className="inline-flex items-center gap-2 rounded-2xl bg-rose-600 px-4 py-2 text-sm font-black text-white hover:bg-rose-700"
                        >
                          <ClipboardCheck className="h-4 w-4" /> تأكيد الخروج وإقفال الطلب
                        </button>
                      </div>
                    )}
                  </div>
      ...
      {canCreate && selectedPass && !['released-teacher', 'completed', 'cancelled'].includes(String(selectedPass?.status || '')) && (
              ...
              <div className="rounded-2xl bg-slate-50 p-4 ring-1 ring-slate-200">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div className="col-span-2">
                    <span className="text-xs text-slate-500 block">المعلم</span>
                    {selectedPass.teacherName ? (
                      <span className="font-black text-slate-900">{selectedPass.teacherName}</span>
                    ) : (
                      <span className="inline-flex items-center gap-1 rounded-full bg-amber-100 px-2 py-0.5 text-xs font-black text-amber-800 ring-1 ring-amber-300">⚠️ لم يُحدد بعد</span>
                    )}
                  </div>
                  ...
                  <div className="col-span-2"><span className="text-xs text-slate-500 block">السبب</span><span className="font-bold text-slate-700">{selectedPass.reason || '—'}</span></div>
                  {selectedPass.guardianName && <div className="col-span-2"><span className="text-xs text-slate-500 block">ولي الأمر</span><span className="font-bold text-slate-700">{selectedPass.guardianName} {selectedPass.guardianMobile ? `— ${selectedPass.guardianMobile}` : ''}</span></div>}
                </div>
              </div>
              {/* === قسم تعيين المعلم — يظهر فقط عندما لا يوجد معلم === */}
              {!selectedPass.teacherUserId && canCreate && (
                <div className="rounded-2xl border-2 border-amber-300 bg-amber-50 p-4">
                  <div className="mb-3 flex items-center gap-2">
                    <span className="text-lg">👨‍🏫</span>
                    <div>
                      <div className="text-sm font-black text-amber-900">تعيين المعلم المختص</div>
                      <div className="text-xs text-amber-700">هذا الطلب من ولي الأمر — اختر المعلم المختص ليتم إشعاره</div>
                    </div>
                  </div>
                  <div className="flex flex-wrap gap-2 items-end">
                    <div className="flex-1 min-w-[180px]">
                      <label className="block text-xs font-bold text-amber-800 mb-1">اختر المعلم</label>
                      <select
                        value={assignTeacherUserId}
                        onChange={(e) => setAssignTeacherUserId(e.target.value)}
                        className="w-full rounded-xl border border-amber-300 bg-white px-3 py-2 text-sm font-bold text-slate-800 focus:outline-none focus:ring-2 focus:ring-amber-400"
                      >
                        <option value="">— اختر معلم —</option>
                        {teacherUsers.map((t) => (
                          <option key={t.id} value={t.id}>{t.name || t.fullName || t.username} {t.className || t.companyName ? `(فص: ${t.className || t.companyName})` : ''}</option>
                        ))}
                      </select>
                    </div>
                    <button
                      onClick={() => assignTeacherToPass(selectedPass)}
                      disabled={!assignTeacherUserId || assigningTeacher}
                      className={`inline-flex items-center gap-2 rounded-xl px-4 py-2 text-sm font-black text-white transition-all ${
                        !assignTeacherUserId || assigningTeacher ? 'bg-amber-300 cursor-not-allowed' : 'bg-amber-600 hover:bg-amber-700'
                      }`}
                    >
                      {assigningTeacher ? 'جارِ التعيين...' : '✔ تعيين وإرسال إشعار'}
                    </button>
                  </div>
                  {assignTeacherStatus && (
                    <div className={`mt-2 rounded-xl px-3 py-2 text-sm font-bold ${
                      assignTeacherStatus.startsWith('✅') ? 'bg-emerald-50 text-emerald-800 ring-1 ring-emerald-200' : 'bg-rose-50 text-rose-800 ring-1 ring-rose-200'
                    }`}>{assignTeacherStatus}</div>
                  )}
                </div>
              )}
            </div>
        ...
        <SectionCard title="إنشاء استئذان جديد" icon={Plus} description="اختر الطالب والمعلم والوجهة، ثم أنشئ الطلب ليظهر معه رابط خاص يمكن مشاركته مع المعلم.">
          <div className="grid gap-4 lg:grid-cols-2">
            <Select label="الفصل" value={form.classFilter || ''} onChange={(e) => setForm((prev) => ({ ...prev, classFilter: e.target.value, studentId: '' }))}>
              ...
              {(() => { const seen = new Map(); students.forEach((s) => { const co = getStudentGroupingLabel(s, selectedSchool); if (co && !seen.has(co)) seen.set(co, s.className || s.classroomName || ""); }); return [...seen.entries()].sort((a, b) => String(a[0]).localeCompare(String(b[0]), "ar")).map(([co, cn]) => <option key={co} value={co}>{cn ? cn + " — " + co : co}</option>); })()}
            </Select>
            ...
            <Select label="طريقة إشعار المعلم" value={form.sendChannel} onChange={(e) => setForm((prev) => ({ ...prev, sendChannel: e.target.value }))}>
              <option value="system">إرسال من النظام (يظهر في جرس المعلم)</option>
              <option value="manual">واتساب يدوي (يفتح واتساب تلقائياً)</option>
              <option value="none">بدون إرسال (يدوياً لاحقاً)</option>
            </Select>
          ...
          <div className="mt-4 flex flex-wrap gap-3">
            <button onClick={handleCreate} className="inline-flex items-center gap-2 rounded-2xl bg-sky-700 px-5 py-3 font-bold text-white"><Plus className="h-4 w-4" /> {form.sendChannel === 'system' ? 'إنشاء وإرسال للمعلم' : form.sendChannel === 'manual' ? 'إنشاء وفتح واتساب' : 'إنشاء الطلب'}</button>
