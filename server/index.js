...
// ✅ إصلاح أمان: نسخة احتياطية تلقائية من DB قبل تطبيق أي migration
try {
  if (existsSync(DB_PATH)) {
    const { copyFileSync: _cpSync } = await import('node:fs');
    const _ts = new Date().toISOString().replace(/[:.]/g, '-').slice(0, 19);
    const _backupPath = DB_PATH + '.pre-migration-' + _ts + '.bak';
    _cpSync(DB_PATH, _backupPath);
    console.log('[startup] ✅ نسخة احتياطية قبل migration:', _backupPath);
  }
} catch (_backupErr) {
  console.warn('[startup] ⚠️ تعذر إنشاء نسخة احتياطية قبل migration:', _backupErr?.message);
}
...
async function ensureStateMigrations() {
  ...
  try {
    ...
    // ✅ إصلاح أمان: لا نمسح البيانات عند فشل المايغريشن
    console.error("[migration] فشل في تطبيق المايغريشن، تم الحفاظ على البيانات:", err?.message || err);
    try { audit(null, "migrate_state_failed", { error: String(err?.message || err) }); } catch {}
  }
}
...
async function saveSharedState(state, actor = null) {
  ...
  // ===== حماية جذرية شاملة: منع حذف الطلاب والشركات والهيكل داخل المدارس =====
  if (Array.isArray(state.schools) && Array.isArray(current.schools)) {
    for (const incomingSchool of state.schools) {
      const serverSchool = current.schools.find(s => s.id === incomingSchool.id);
      if (!serverSchool) continue;

      // --- حماية students المباشرة ---
      ...
      // --- حماية companies ---
      const serverCompanies = Array.isArray(serverSchool.companies) ? serverSchool.companies : [];
      const incomingCompanies = Array.isArray(incomingSchool.companies) ? incomingSchool.companies : [];
      if (serverCompanies.length > 0 && incomingCompanies.length === 0) {
        console.warn(`[PROTECT-COMPANIES] Prevented deletion of ${serverCompanies.length} companies in school ${serverSchool.name}`);
        incomingSchool.companies = serverCompanies;
      } else if (serverCompanies.length > 0 && incomingCompanies.length < serverCompanies.length * 0.5) {
        console.warn(`[PROTECT-COMPANIES] Prevented major company loss in ${serverSchool.name}: ${incomingCompanies.length} < ${serverCompanies.length}`);
        const incomingCIds = new Set(incomingCompanies.map(c => String(c.id)));
        const missingCompanies = serverCompanies.filter(c => !incomingCIds.has(String(c.id)));
      ...
      const serverStructureStudents = serverClassrooms.reduce((sum, c) => sum + (Array.isArray(c.students) ? c.students.length : 0), 0);
      const incomingStructureStudents = incomingClassrooms.reduce((sum, c) => sum + (Array.isArray(c.students) ? c.students.length : 0), 0);
      if (serverStructureStudents > 0 && incomingStructureStudents === 0) {
        console.warn(`[PROTECT-STRUCTURE] Prevented deletion of ${serverStructureStudents} structure students in school ${serverSchool.name}`);
        if (!incomingSchool.structure) incomingSchool.structure = {};
        incomingSchool.structure.classrooms = serverClassrooms;
      } else if (serverStructureStudents > 0 && incomingStructureStudents < serverStructureStudents * 0.5) {
        console.warn(`[PROTECT-STRUCTURE] Prevented major structure student loss in ${serverSchool.name}: ${incomingStructureStudents} < ${serverStructureStudents}`);
        const incomingCRIds = new Set(incomingClassrooms.map(c => String(c.id)));
  ...
  for (const admin of admins) {
    try {
      if (channels.email && admin.email) {
        await sendAuthEmail(authSettings.email || {}, admin.email, '000000', { customSubject: 'تنبيه أمني في منصة المدرسة المثالية', customText: message });
      }
    } catch {}
    try {
...
async function sendAuthEmail(config, to, code, context = {}) {
  ...
  await transporter.sendMail({
    from: `"${config.fromName || 'منصة المدرسة المثالية'}" <${config.fromEmail}>`,
    to,
...
async function runAuthDeliveryTest(state, actor, body = {}) {
  ...
  const integration = authSettings.integrations || defaultSettings.auth.integrations;
  if (delivery === 'sms') {
    const sent = await sendSmsMessage(integration.sms || {}, targetPhone, `رسالة اختبار من منصة المدرسة المثالية للتحقق من قناة OTP. الوقت: ${new Date().toLocaleString('ar-SA')}`);
  ...
  if (delivery === 'whatsapp') {
    const sent = await sendWhatsappCloudMessage(integration.whatsapp || {}, targetPhone, `رسالة اختبار من منصة المدرسة المثالية للتحقق من قناة OTP.
...
async function testSchoolMessagingIntegration(state, schoolId, channel, settingsOverride = null) {
  ...
  if (channel === 'whatsapp') {
    ...
    try {
      const sent = await sendWhatsappCloudMessage(cfg, cfg.testRecipient, 'رسالة اختبار من منصة المدرسة المثالية. تم التحقق من الربط بنجاح.');
  ...
  if (channel === 'sms') {
    ...
    try {
      const sent = await sendSmsMessage(cfg, cfg.testRecipient, 'رسالة اختبار من منصة المدرسة المثالية. تم التحقق من الربط بنجاح.');
  ...
  try {
    ...
    if (schoolDeviceMatch && req.method === 'POST') {
      ...
      const screen = {
        ...
        widgets: {
          ...
          parentPortalSummary: body.widgets?.parentPortalSummary !== false,
          lessonAttendanceSummary: body.widgets?.lessonAttendanceSummary !== false,
          rewardStoreSummary: body.widgets?.rewardStoreSummary !== false,
          actionStats: body.widgets?.actionStats !== false,
          teacherActivity: body.widgets?.teacherActivity !== false,
          rewardCategoryBreakdown: body.widgets?.rewardCategoryBreakdown !== false,
          topTeachers: body.widgets?.topTeachers !== false,
          nafisQuiz: body.widgets?.nafisQuiz === true,
          nafisLeaderboard: body.widgets?.nafisLeaderboard === true,
        },
    ...
    if (schoolDeviceUpdateMatch && req.method === 'PATCH') {
      ...
      if (body.widgets && typeof body.widgets === 'object') {
        ...
        screen.widgets.parentPortalSummary = body.widgets.parentPortalSummary !== false;
        screen.widgets.lessonAttendanceSummary = body.widgets.lessonAttendanceSummary !== false;
        screen.widgets.rewardStoreSummary = body.widgets.rewardStoreSummary !== false;
        screen.widgets.actionStats = body.widgets.actionStats !== false;
    ...
    if (reqUrl.pathname === '/api/admin/parent-primary-requests' && req.method === 'GET') {
      ...
      // ✅ إصلاح: جلب البيانات الحقيقية بدل القوائم الفارغة الصلبة
      // تحديد نطاق المدارس حسب دور المستخدم
      const schoolId = Number(reqUrl.searchParams.get('schoolId') || actor.schoolId || 0);
      const scopedSchools = actor.role === 'superadmin'
        ? (schoolId ? state.schools.filter((s) => Number(s.id) === schoolId) : state.schools)
        : state.schools.filter((s) => Number(s.id) === Number(actor.schoolId || 0));

      // جلب الاستئذانات المعلقة (parent-requested / created) من جميع المدارس المعنية
      const pendingLeavePasses = scopedSchools.flatMap((school) =>
        (Array.isArray(school.leavePasses) ? school.leavePasses : [])
          .filter((p) => ['parent-requested', 'created', 'sent-system', 'sent-manual'].includes(String(p.status || '')))
          .map((p) => ({ ...p, schoolName: school.name || '' }))
      ).slice(0, 50);

      // جلب طلبات تغيير الرقم الأساسي المعلقة
      const pendingPrimaryRequests = [];
      try {
        const parents = await listParentAccounts(state, actor);
        const scopedSchoolIds = scopedSchools.map((s) => Number(s.id));
        for (const parent of parents) {
          const req = parent.primaryChangeRequest;
          if (!req || String(req.status || '') !== 'pending') continue;
          const parentSchools = Array.isArray(parent.schoolIds) ? parent.schoolIds.map(Number) : [Number(parent.schoolId || 0)];
          if (scopedSchoolIds.some((id) => parentSchools.includes(id))) {
            pendingPrimaryRequests.push({
              id: req.id || parent.mobile,
              mobile: parent.mobile,
              guardianName: parent.guardianName || 'ولي الأمر',
              requestedMobile: req.requestedMobile || '',
              status: 'pending',
              requestedAt: req.requestedAt || '',
            });
          }
        }
      } catch (_e) { /* تجاهل الخطأ — سيرجع الاستئذانات على الأقل */ }

      // جلب تنبيهات ولي الأمر الأخيرة من app_meta
      const alertsFromMeta = [];
      try {
        for (const school of scopedSchools) {
          const schoolAlerts = await getParentPrimaryChangeAlerts(school.id);
          alertsFromMeta.push(...schoolAlerts.slice(0, 10).map((a) => ({
            id: a.id,
            title: a.message || 'تغيير رقم ولي الأمر',
            body: (a.guardianName || 'ولي الأمر') + (a.studentNames?.length ? ' — ' + a.studentNames.join('، ') : ''),
            time: a.createdAt || '',
      ...
      return sendJson(res, 200, {
        ok: true,
        requests: pendingPrimaryRequests,
        leavePasses: pendingLeavePasses,
        alerts: alertsFromMeta.slice(0, 20),
...
الصلاحية: 10 دقائق.`);
    ...
    if (reqUrl.pathname === '/api/parent/change-primary/request-otp' && req.method === 'POST') {
      ...
      const delivery = await trySendOtpToChannel({
        channel: 'sms',
        recipient: mobile,
        message: `رمز التحقق لتحديث الرقم الأساسي في منصة المدرسة المثالية هو: ${code}`,
      });
...
<head>
  ...
  <title>اعتماد طلبات أولياء الأمور - منصة المدرسة المثالية</title>
  <style>
