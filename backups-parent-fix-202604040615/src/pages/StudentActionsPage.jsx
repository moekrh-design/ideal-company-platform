        ...
        <>
          ...
          {identifyMethod === 'manual' ? (
            <div className="rounded-2xl bg-white p-4 ring-1 ring-slate-200">
              ...
              <datalist id="manual-student-options">{teacherManualStudentOptions.map((student) => <option key={student.id} value={student.studentNumber || student.barcode || student.name || student.fullName}>{student.name || student.fullName || "طالب"} — {student.nationalId || student.barcode}</option>)}</datalist>
              <div className="mt-3">
                <Select label="أو اختر من قائمة الطلاب المسجلة" value={manualSelectedStudentId} onChange={(e) => handleManualStudentSelection(e.target.value)}>
                  ...
                  {teacherManualStudentOptions.map((student) => (
                    <option key={`manual-${student.id}`} value={student.id}>{student.name || student.fullName || "طالب"} — {getStudentCompanyName(student, selectedSchool)}</option>
                  ))}
                </Select>
              </div>
