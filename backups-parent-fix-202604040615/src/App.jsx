...
export default function App() {
  ...
  useEffect(() => {
    ...
    const loadHeaderAlerts = async () => {
      ...
      try {
        ...
        const localItems = [...leavePassAlerts, ...teacherNotifs, ...(notifications || []).filter((note) => /فشل|تعذر|ولي الأمر|أولياء الأمور|الرقم الأساسي|بوابة ولي الأمر/.test(String(note?.title || '') + ' ' + String(note?.body || '')))].slice(0, 6).map((note, index) => ({
  ...
  const handleCreateLeavePass = async (payload = {}) => {
    ...
    setTimeout(persistLeavePassesToServer, 500);
    // إشعار مستهدف للمعلم (يظهر في الجرس)
    setNotifications((prev) => [{
      id: Date.now() + 1,
      title: '🚨 طلب استئذان جديد',
      body: `الطالب ${leavePass.studentName} — ${leavePass.destination === 'agent' ? 'الوكيل' : leavePass.destination === 'counselor' ? 'المرشد' : 'ولي الأمر'}`,
      time: 'الآن',
      type: 'leavePass',
      leavePassId: leavePassId,
      forTeacherIds: [teacher.id],
      urgent: true,
      studentName: leavePass.studentName,
      destination: leavePass.destination,
    }, ...prev].slice(0, 30));
  ...
  const handleMarkLeavePassViewed = (leavePassId) => {
    ...
    if (changed) setTimeout(persistLeavePassesToServer, 500);
  ...
  const handleUpdateLeavePassStatus = (leavePassId, status = 'completed') => {
    ...
    if (changed) {
      pushNotification('الاستئذان', status === 'completed' ? 'تم اعتماد تنفيذ الاستئذان.' : 'تم تحديث حالة الاستئذان.');
      setTimeout(persistLeavePassesToServer, 500);
    }
