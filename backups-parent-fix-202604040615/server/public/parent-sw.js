// Service Worker - بوابة ولي الأمر PWA
...
self.addEventListener('fetch', (event) => {
  ...
  // صفحة /parent: Network First مع Fallback للـ cache
...
self.addEventListener('push', (event) => {
  let data = { title: 'بوابة ولي الأمر', body: 'لديك إشعار جديد' };
  try {
    data = event.data.json();
  } catch {}

  event.waitUntil(
    self.registration.showNotification(data.title || 'بوابة ولي الأمر', {
      body: data.body || '',
