  ...
  useEffect(() => {
    ...
    const loadHeaderAlerts = async () => {
      ...
      try {
        ...
        const localItems = [...teacherLeavePassAlerts, ...leavePassAlerts, ...teacherNotifs, ...(notifications || []).filter((note) => /فشل|تعذر|ولي الأمر|أولياء الأمور|الرقم الأساسي|بوابة ولي الأمر/.test(String(note?.title || '') + ' ' + String(note?.body || '')))].slice(0, 8).map((note, index) => ({
