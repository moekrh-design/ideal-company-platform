...
import { Input, FieldError } from '../components/ui/FormElements';
...
function CompaniesPage({ selectedSchool, onAddCompany, onDeleteCompany, onAwardInitiative }) {
  ...
  const submit = (e) => {
    e.preventDefault();
    const ok = validate({
      name:      [validators.required('اسم الشركة/الفصل')],
      className: [validators.required('رقم الفصل')],
    }, form);
    if (!ok) return;
    onAddCompany(form);
          ...
          <div className="space-y-6 xl:col-span-2">
            ...
            {hasStructureCompanies ? <div className="rounded-3xl border border-dashed border-sky-200 bg-sky-50 p-5 text-sm leading-7 text-sky-800">يتم الآن اعتماد الفصول والشركات من <span className="font-black">الهيكل المدرسي</span> كمصدر افتراضي. يمكن الإضافة والتعديل من صفحة الهيكل المدرسي للحفاظ على توحيد البيانات.</div> : <form onSubmit={submit} className="rounded-3xl bg-slate-50 p-5 ring-1 ring-slate-200">
              ...
              <div className="grid grid-cols-1 gap-4 md:grid-cols-3">
                <Input required label="اسم الشركة" value={form.name} error={errors.name} onChange={(e) => { setForm({ ...form, name: e.target.value }); clearError('name'); }} placeholder="مثال: شركة الريادة" />
                <Input required label="الفصل" value={form.className} error={errors.className} onChange={(e) => { setForm({ ...form, className: e.target.value }); clearError('className'); }} placeholder="مثال: 1/3" />
                <Input label="رائد الشركة" value={form.leader} onChange={(e) => setForm({ ...form, leader: e.target.value })} placeholder="اسم الرائد" hint="اختياري" />
              </div>
