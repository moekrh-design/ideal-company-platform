  ...
  if (Array.isArray(state.schools) && Array.isArray(current.schools)) {
    for (const incomingSchool of state.schools) {
      ...
      for (const p of incomingPasses) {
        ...
        const statusOrder = { 'created': 0, 'sent-manual': 1, 'sent-system': 1, 'viewed': 2, 'approved-agent': 3, 'approved-counselor': 3, 'released-guardian': 4, 'completed': 5, 'cancelled': 5 };
        const existingRank = statusOrder[existing.status] ?? 0;
        const incomingRank = statusOrder[p.status] ?? 0;
        if (incomingRank > existingRank) { merged.set(String(p.id), p); }
...
function mergeActionItems(serverItems = [], incomingItems = []) {
  ...
  for (const item of incomingItems) {
    ...
    if (existing) {
      ...
      merged.set(key, { ...existing, ...item, title: item.title || item.label || existing.title || existing.label, label: item.label || item.title || existing.label || existing.title });
    } else {
...
function applyStudentActionToState(state, schoolId, payload, actor) {
  ...
  if (!student && Array.isArray(school?.structure?.classrooms)) {
    for (const classroom of school.structure.classrooms) {
      ...
      const found = classroom.students.find((item) => Number(item.id) === Number(payload.studentId));
...
الصلاحية: 10 دقائق.`);
    ...
    if (reqUrl.pathname === '/api/leave-passes/save' && req.method === 'POST') {
      ...
      const merged = serverPasses.map((sp) => {
        ...
        // Client version is newer if updatedAt/completedAt is more recent
    ...
    if (reqUrl.pathname === '/api/leave-passes/pending' && req.method === 'GET') {
      ...
      const myPasses = actor.role === 'teacher'
        ? activePasses.filter((p) => String(p.teacherUserId || '') === String(actor.id || ''))
        : activePasses;
